...
extension Z80 {
    func initOpcodeTableCB(_ opcodes: inout OpcodeTable) {
        ...
        opcodes[0x0E] = { // RRC (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .rrc, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x0F] = { // RRC A
        ...
        opcodes[0x16] = { // RL (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .rl, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x17] = { // RL A
        ...
        opcodes[0x1E] = { // RR (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .rr, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x1F] = { // RR A
        ...
        opcodes[0x26] = { // SLA (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .sla, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x27] = { // SLA A
        ...
        opcodes[0x2E] = { // SRA (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .sra, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x2F] = { // SRA A
        ...
        opcodes[0x36] = { // SLS (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .sls, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x37] = { // SLS A
        ...
        opcodes[0x3E] = { // SRL (HL)
            self.t_cycle += 7
            var data = self.dataBus.read(self.regs.hl)
            data = self.ulaCall(data, 1, ulaOp: .srl, ignoreCarry: false)
            self.dataBus.write(self.regs.hl, value: data)
        }
        opcodes[0x3F] = { // SRL A
        ...
        opcodes[0x40] = { // BIT 0,B
            let _ = self.ulaCall(self.regs.b, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x41] = { // BIT 0,C
            let _ = self.ulaCall(self.regs.c, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x42] = { // BIT 0,D
            let _ = self.ulaCall(self.regs.d, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x43] = { // BIT 0,E
            let _ = self.ulaCall(self.regs.e, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x44] = { // BIT 0,H
            let _ = self.ulaCall(self.regs.h, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x45] = { // BIT 0,L
            let _ = self.ulaCall(self.regs.l, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x46] = { // BIT 0,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x47] = { // BIT 0,A
            let _ = self.ulaCall(self.regs.a, 0, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x48] = { // BIT 1,B
            let _ = self.ulaCall(self.regs.b, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x49] = { // BIT 1,C
            let _ = self.ulaCall(self.regs.c, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4A] = { // BIT 1,D
            let _ = self.ulaCall(self.regs.d, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4B] = { // BIT 1,E
            let _ = self.ulaCall(self.regs.e, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4C] = { // BIT 1,H
            let _ = self.ulaCall(self.regs.h, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4D] = { // BIT 1,L
            let _ = self.ulaCall(self.regs.l, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4E] = { // BIT 1,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x4F] = { // BIT 1,A
            let _ = self.ulaCall(self.regs.a, 1, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x50] = { // BIT 2,B
            let _ = self.ulaCall(self.regs.b, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x51] = { // BIT 2,C
            let _ = self.ulaCall(self.regs.c, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x52] = { // BIT 2,D
            let _ = self.ulaCall(self.regs.d, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x53] = { // BIT 2,E
            let _ = self.ulaCall(self.regs.e, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x54] = { // BIT 2,H
            let _ = self.ulaCall(self.regs.h, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x55] = { // BIT 2,L
            let _ = self.ulaCall(self.regs.l, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x56] = { // BIT 2,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x57] = { // BIT 2,A
            let _ = self.ulaCall(self.regs.a, 2, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x58] = { // BIT 3,B
            let _ = self.ulaCall(self.regs.b, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x59] = { // BIT 3,C
            let _ = self.ulaCall(self.regs.c, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5A] = { // BIT 3,D
            let _ = self.ulaCall(self.regs.d, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5B] = { // BIT 3,E
            let _ = self.ulaCall(self.regs.e, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5C] = { // BIT 3,H
            let _ = self.ulaCall(self.regs.h, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5D] = { // BIT 3,L
            let _ = self.ulaCall(self.regs.l, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5E] = { // BIT 3,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x5F] = { // BIT 3,A
            let _ = self.ulaCall(self.regs.a, 3, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x60] = { // BIT 4,B
            let _ = self.ulaCall(self.regs.b, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x61] = { // BIT 4,C
            let _ = self.ulaCall(self.regs.c, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x62] = { // BIT 4,D
            let _ = self.ulaCall(self.regs.d, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x63] = { // BIT 4,E
            let _ = self.ulaCall(self.regs.e, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x64] = { // BIT 4,H
            let _ = self.ulaCall(self.regs.h, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x65] = { // BIT 4,L
            let _ = self.ulaCall(self.regs.l, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x66] = { // BIT 4,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x67] = { // BIT 4,A
            let _ = self.ulaCall(self.regs.a, 4, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x68] = { // BIT 5,B
            let _ = self.ulaCall(self.regs.b, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x69] = { // BIT 5,C
            let _ = self.ulaCall(self.regs.c, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6A] = { // BIT 5,D
            let _ = self.ulaCall(self.regs.d, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6B] = { // BIT 5,E
            let _ = self.ulaCall(self.regs.e, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6C] = { // BIT 5,H
            let _ = self.ulaCall(self.regs.h, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6D] = { // BIT 5,L
            let _ = self.ulaCall(self.regs.l, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6E] = { // BIT 5,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x6F] = { // BIT 5,A
            let _ = self.ulaCall(self.regs.a, 5, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x70] = { // BIT 6,B
            let _ = self.ulaCall(self.regs.b, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x71] = { // BIT 6,C
            let _ = self.ulaCall(self.regs.c, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x72] = { // BIT 6,D
            let _ = self.ulaCall(self.regs.d, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x73] = { // BIT 6,E
            let _ = self.ulaCall(self.regs.e, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x74] = { // BIT 6,H
            let _ = self.ulaCall(self.regs.h, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x75] = { // BIT 6,L
            let _ = self.ulaCall(self.regs.l, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x76] = { // BIT 6,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x77] = { // BIT 6,A
            let _ = self.ulaCall(self.regs.a, 6, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x78] = { // BIT 7,B
            let _ = self.ulaCall(self.regs.b, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x79] = { // BIT 7,C
            let _ = self.ulaCall(self.regs.c, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7A] = { // BIT 7,D
            let _ = self.ulaCall(self.regs.d, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7B] = { // BIT 7,E
            let _ = self.ulaCall(self.regs.e, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7C] = { // BIT 7,H
            let _ = self.ulaCall(self.regs.h, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7D] = { // BIT 7,L
            let _ = self.ulaCall(self.regs.l, 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7E] = { // BIT 7,(HL)
            self.t_cycle += 4
            let _ = self.ulaCall(self.dataBus.read(self.regs.hl), 7, ulaOp: .bit, ignoreCarry: false)
        }
        opcodes[0x7F] = { // BIT 7,A
            let _ = self.ulaCall(self.regs.a, 7, ulaOp: .bit, ignoreCarry: false)
        }
